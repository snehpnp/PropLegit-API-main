...
var POSTTrustDocument = (req, res) => {

    if (!req.file || !req.params.TrustID) {
        var errMessage = `Please Upload Document File and TrustID`;
        ...
        checkFile(req.params.TrustID, req.body.DocumentTypeId, req.body.FileName)
            .then(data => {
                ...
                UploadFile(req.file, req.params.TrustID, req.body.DocumentTypeId, req.body.FileName)
                    .then(result => {
                        var model = new modelBuilder().buildModel(req.body, new PropertyDocumentModel());

                        model.TrustID = req.params.TrustID;
                        ...
                        runStoredProcedure(spName, inputparams)
                            .then(result => {
                                res.status(200).send(new modelBuilder().buildModel({ status: 200, data: result.recordset, message: "Trust Document successfully Uploaded!!" }, new ResponseModel()));
                            })
                            .catch(err => {
...
var GETtrustDocumentByID = (req, res) => {

    var MsSqlQuery = SqlString.format(`Select t.*,dd.DropDownValue  from ${viewName} t JOIN tbl_Dropdownitem  dd ON (dd.DropDownItemId = t.DocumentSubTypeId) where TrustID = ${req.params.TrustID}`);
    ...
    executeQuery(MsSqlQuery)
        ...
        .catch(err => {
            console.log('err: ', err);
            var errMessage = `An error occurred during Fetching Trust's Document Details`;
...
router.post('/api/trust/document/add/:TrustID', upload.single('uploadfile'), POSTTrustDocument);
router.get('/api/trust/document/View/:TrustID', GETtrustDocumentByID);
